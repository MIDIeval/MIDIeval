...
public class UsbCommunication 
{
	...
	private final UsbDeviceConnection usbDeviceConnection_m; 
	...
	public UsbCommunication(final UsbDevice usbDevice, final UsbDeviceConnection usbDeviceConnection, final UsbEndpoint usbEndpoint, final UsbInterface usbInterface)
	{
		...
		Log.i(com.beta.util.Constants.TAG, "Usb Device: " + this.getUsbDevice_m() + ", Usb Interface: " + this.usbInterface_m + ", Usb Endpoint: " + this.usbEndpoint_m + ", Usb Device Connection: " + this.getUsbDeviceConnection() );
		
		this.getUsbDeviceConnection().claimInterface(this.usbInterface_m, true);//claimInterface(interface, force):
	...
	public final boolean fn_WriteUSBMessage(byte[] usbDataPacket){
		...
		usbDeviceBulkTransferStatus_f = this.getUsbDeviceConnection().bulkTransfer(this.usbEndpoint_m, usbDataPacket, usbDataPacket.length, 0);
	...
	public final byte[] fn_ReadUSBMessage(){
		...
		try{
			usbDeviceBulkTransferStatus_f = this.getUsbDeviceConnection().bulkTransfer(this.usbEndpoint_m, usbDataPacket_f, usbDataPacket_f.length, 0);
			if( usbDeviceBulkTransferStatus_f > 0 ){			
